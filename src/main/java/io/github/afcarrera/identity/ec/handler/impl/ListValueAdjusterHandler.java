 ...
 * <p>This class extends {@link AbstractIdentityHandler} and adjusts values in the multiplication list
 * based on the specified conditions, then sets the modified list in the context.</p>
 */
public class ListValueAdjusterHandler extends AbstractIdentityHandler {
     ...
     * <p>If the value is greater than or equal to maxValue, subtracts subtrahend from it.</p>
     * <p>The modified list is then set in the context.</p>
     * @param context The identity document to be processed.
     */
    @Override
    public void process(IdentityDocument context) {
        if (hasToAdjust){
            List<Integer> modList = context.getMultiplicationList().stream()
